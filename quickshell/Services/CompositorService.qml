...
Singleton {
    ...
    property var monitors: [] // {id, name, scale, isFocused}
    ...
    function updateHyprlandMonitors() {
        ...
        try {
            ...
            for (var i = 0; i < rawMonitors.length; i++) {
                ...
                monitorsList.push({
                    "id": monitor.id || "",
                    "name": monitor.name || "",
                    "scale": monitor.scale  || Settings.data.defaultScale,
                    "isFocused": monitor.focused
                });
            }
            ...
            Logger.error("Compositor", "Error updating Hyprland monitors:", e);
        }
    ...
    // Get available monitors and their data
